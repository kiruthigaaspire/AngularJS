...
.controller(
    'RegisterController',
    function RegisterController($scope, $http, Auth,$location, $rootScope, domain) {
      ...
      $scope.register = function() {
        ...
        if($scope.user === undefined) {
          ...
          $scope.errors = true;
          $scope.messages = ['Please enter a name'];
        } else if(!$scope.emailRegx.test($scope.user['email'])) { console.log('hello');
          $scope.errors = true;
      ...
      $scope.change_password=function(){
              ...
              if($scope.updateuser['currentpassword']==='' && $scope.updateuser['confirmation']==='' && $scope.updateuser['newpassword']===''){
                 ...
                 $scope.errors = true;
                    ...
                    $scope.messages = ['Current password should have  8 or above characters'];
                    $scope.errors = true;
              } else if($scope.updateuser['newpassword'].length <8) {
                    $scope.messages = ['New password should have atleast 8 characters'];
                    $scope.errors = true;
              } 
              else if($scope.updateuser['confirmation'].length <8 ) {
                    $scope.messages = ['Confirmation password should have atleast 8 characters'];
                    $scope.errors = true;
              }  
                    ...
                    $scope.messages = ['Current Password and New Passwords cannot be same'];
                    $scope.errors = true;
              }  else {
                
                 var updateuserInfo = {oldpassword:$scope.updateuser['currentpassword'],newpassword:$scope.updateuser['newpassword'],updatecustomerid:$scope.customerId,email:$scope.customerEmail};
                 $rootScope.loading = true;
                 $http.post(domain+'/shop/api/rest/customer', updateuserInfo)
                 .success(function(data, status, headers, config) {              
                if(data.change_status=== 1){ 
                  //$scope.loginUser = username = $scope.updateuser['username'];
                  //document.cookie="loginUser="+username+";";
                  $scope.showMyAccountForm = false;
                  $scope.errors = false;                
                  $scope.messages = [data.message];
                  $scope.linktoredirect = true;
                }else{ 
                  $scope.showMyAccountForm = true;
                  $scope.errors = true;                
                  $scope.messages = [data.message];
        ...
        if($scope.updateuser['username'] === null || $scope.updateuser['username'] === '' ) {
            ...
            $http.post(domain+'/book_library/UserController.php/user/update', updateuserInfo)
              .success(function(data, status, headers, config) { 
                ...
                var messages = [];
                angular.forEach(data.message.error, function(error, key) {
                  if (error.code != 500) { 
                    this.push(error.message);
                  }
                }, messages);
